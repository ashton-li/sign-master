...
assert.match(readFileSync(previewJs, 'utf8'), /value:"jpg",label:"JPEG".*value:"pdf",label:"PDF".*value:"png",label:"PNG"/, '图片导出格式顺序不是 JPEG、PDF、PNG')
  ...
  assert.match(scannerLogic, /scanPdf\.js/, '扫描完成没有使用分包内 JPEG-PDF 生成器')
  assert.doesNotMatch(scannerLogic, /core\/export\/imagePdf\.js/, '扫描页仍跨包加载 JPEG-PDF 生成器')
  const scanPdfLogic = readFileSync(new URL('../../dist/build/mp-weixin/pages/sign/scanPdf.js', import.meta.url), 'utf8')
  assert.match(scanPdfLogic, /PDF-1\.4/, '扫描分包缺少 PDF 生成实现')
...
if (process.env.MP_STATIC_ONLY === '1') {
  console.log(JSON.stringify({ staticChecks:true, scanPdfInSubpackage:true, appid:appJson.appid }, null, 2))
  process.exit(0)
}
const routes = [
