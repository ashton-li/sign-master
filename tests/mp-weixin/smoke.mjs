...
assert.doesNotMatch(settingsWxml, /推荐签字大师|promotion-panel/, '设置页仍包含已删除的推荐分享入口')
  ...
  assert.match(scannerCoreLogic, /createOffscreenCanvas/, '扫描器没有编译微信 2D 离屏原图读取')
  assert.match(scannerCoreLogic, /scanResolutionMode/, '扫描器没有编译原图读取失败后的兼容降级')
  assert.doesNotMatch(scannerCoreLogic, /读取原图失败，请重新拍摄后重试/, '扫描器仍会因原分辨率二次读取失败而拒绝页面')
