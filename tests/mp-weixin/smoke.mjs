...
const previewJs = new URL('../../dist/build/mp-weixin/pages/sign/preview.js', import.meta.url)
  ...
  const scannerCoreJs = new URL('../../dist/build/mp-weixin/core/vision/documentScanner.js', import.meta.url)
const previewLogic = () => readFileSync(previewJs, 'utf8')
const templateCardWxml = new URL('../../dist/build/mp-weixin/components/TemplateCard.wxml', import.meta.url)
assert.match(previewLogic(), /发送给好友/, 'generated share action is missing')
assert.match(previewLogic(), /分享文件已生成/, 'on-demand share generation status is missing')
assert.doesNotMatch(previewLogic(), /scheduleSharePreparation|准备分享/, 'preview still generates an export on page entry')
...
assert.doesNotMatch(settingsWxml, /推荐签字大师|promotion-panel/, '设置页仍包含已删除的推荐分享入口')
  ...
  assert.doesNotMatch(scannerStyles, /process-canvas[^}]*width:1px[^}]*height:1px/, '扫描处理画布仍被压缩为 1px')
  assert.match(scannerMarkup, /scanProcessCanvas[^>]*style=/, '扫描处理画布没有同步实际显示尺寸')
  const scannerCoreLogic = readFileSync(scannerCoreJs, 'utf8')
  assert.match(scannerCoreLogic, /按取景框裁切/, '扫描器没有编译取景框直接裁切流程')
  assert.match(scannerCoreLogic, /source-direct/, '扫描器没有保留原图像素尺寸')
  assert.doesNotMatch(scannerCoreLogic, /createOffscreenCanvas/, '扫描页生产包仍包含已停用的自动边沿识别代码')
