...
describe('mp-weixin compatibility guards', () => {
  ...
  it('packages independent capacity/about pages and keeps mini-program scanning lightweight', () => {
    ...
    expect(scanner).toContain('cropImageToGuideFrame')
    expect(scanner).toContain('measureGuideFrame')
    expect(scanner).not.toContain('scanDocumentImage(source')
    expect(scanner).toContain('quality:1')
    expect(scanner).toContain('请将纸张四边对齐边框后拍照')
    expect(scanner).toContain('width:82vw;height:116vw')
