    ...
    <template #overlay>
      <view v-if="previewing" class="preview-mask">
        <view class="preview-dialog">
          <view class="preview-head"><text>{{ previewing.name || previewing.label || '签名预览' }}</text></view>
          <view class="large-preview"><image v-if="previewing.snapshot?.pngPath" :src="previewing.snapshot.pngPath" mode="aspectFit" /><SignatureInk v-else canvas-id="library-signature-large" :snapshot="previewing.snapshot" :width="300" :height="150" /></view>
