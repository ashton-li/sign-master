<template>
  <PageShell active="home">
    ...
    <view v-else :class="['home-empty',{ guided:firstUse }]">
      ...
      <view class="guide-arrow" aria-label="指向下方签署按钮"><SvgIcon name="arrowDown" :size="25" color="#5856e0" /></view>
    </view>
  </PageShell>
</template>
...
.brand-halo::before{animation:brandHalo 2.8s ease-out infinite}.brand-halo::after{animation:brandHalo 2.8s .9s ease-out infinite}.brand-halo :deep(.app-logo){animation:brandFloat 2.7s ease-in-out infinite}.empty-pulse::before{animation:guidePulse 2.15s ease-out infinite}.empty-pulse::after{animation:guidePulse 2.15s .58s ease-out infinite}.empty-doc{transform:none}.guide-arrow{margin-top:15px;animation:guideDown 1.15s ease-in-out infinite}@keyframes brandHalo{0%{opacity:.72;transform:scale(.78)}72%,100%{opacity:0;transform:scale(1.14)}}@keyframes brandFloat{0%,100%{transform:translateY(0) scale(1)}50%{transform:translateY(-3px) scale(1.025)}}@media (prefers-reduced-motion:reduce){.brand-halo::before,.brand-halo::after,.brand-halo :deep(.app-logo),.empty-pulse::before,.empty-pulse::after,.guide-arrow{animation:none}}
</style>
