...
export function hmacSha256Hex(secret, message) {
  ...
  if (key.length > blockSize) key = Uint8Array.from(sha256Hex(key).match(/../g).map((hex) => parseInt(hex, 16)))
  ...
  innerPayload.set(inner); innerPayload.set(messageBytes, inner.length)
  const innerHash = Uint8Array.from(sha256Hex(innerPayload).match(/../g).map((hex) => parseInt(hex, 16)))
  const outerPayload = new Uint8Array(outer.length + innerHash.length)
  outerPayload.set(outer); outerPayload.set(innerHash, outer.length)
  return sha256Hex(outerPayload)
}
